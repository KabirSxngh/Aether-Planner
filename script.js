document.addEventListener('DOMContentLoaded', () => {
    ...
    function init() {
        ...
        renderTags();
        bindEvents();
        generateCalendar(); // THE MISSING KEY!
    }
    ...
    // (All other functions are exactly the same as the last version)
    ...
    function applyTheme(type = 'random') { let gradient, accent, primary; const baseHue = Math.floor(Math.random() * 360); if (type === 'dark') { const darkSat = 60 + Math.floor(Math.random() * 20); const darkLight1 = 10 + Math.floor(Math.random() * 5); const darkLight2 = 15 + Math.floor(Math.random() * 5); const accentLight = 60 + Math.floor(Math.random() * 10); primary = `hsl(${baseHue}, ${darkSat}%, ${darkLight1}%)`; const color2 = `hsl(${(baseHue + 40) % 360}, ${darkSat - 10}%, ${darkLight2}%)`; accent = `hsl(${(baseHue + 150) % 360}, ${darkSat + 10}%, ${accentLight}%)`; gradient = `linear-gradient(to right top, ${primary}, ${color2})`; } else { const randSat = 50 + Math.floor(Math.random() * 20); const randLight = 55 + Math.floor(Math.random() * 10); primary = `hsl(${baseHue}, ${randSat}%, ${randLight}%)`; const randColor2 = `hsl(${(baseHue + 40) % 360}, ${randSat - 5}%, ${randLight - 10}%)`; accent = `hsl(${(baseHue + 20) % 360}, ${randSat + 20}%, ${randLight + 5}%)`; gradient = `linear-gradient(to right top, ${primary}, ${randColor2})`; } document.documentElement.style.setProperty('--bg-gradient', gradient); document.documentElement.style.setProperty('--accent-color', accent); if (themeColorMeta) { themeColorMeta.setAttribute('content', primary); } saveState(); }
