document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyTheme(type = 'random') {
        let gradient, accent, primary;
        ...
        if (type === 'dark') {
            ...
            primary = `hsl(${baseHue}, ${darkSat}%, ${darkLight1}%)`;
            const color2 = `hsl(${(baseHue + 40) % 360}, ${darkSat - 10}%, ${darkLight2}%)`;
            accent = `hsl(${(baseHue + 150) % 360}, ${darkSat + 10}%, ${accentLight}%)`;
            gradient = `linear-gradient(to right top, ${primary}, ${color2})`;
        } else { // 'random' (light)
            const randSat = 50 + Math.floor(Math.random() * 20);
            const randLight = 55 + Math.floor(Math.random() * 10);
            primary = `hsl(${baseHue}, ${randSat}%, ${randLight}%)`;
            const randColor2 = `hsl(${(baseHue + 40) % 360}, ${randSat - 5}%, ${randLight - 10}%)`;
            accent = `hsl(${(baseHue + 20) % 360}, ${randSat + 20}%, ${randLight + 5}%)`;
            gradient = `linear-gradient(to right top, ${primary}, ${randColor2})`;
        }
    ...
    // (The rest of the JS file is the same as the final "Director's Cut" version)
